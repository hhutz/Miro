...
bool verbose = false;
bool xmlFormatting = false;
bool logFormatting = false;

std::ostream& operator<< (std::ostream& ostr, CosNotification::EventType const& rhs)
{
  if (!xmlFormatting) {
    ostr << "domain name: " << rhs.domain_name << "    "
	 << "type name: " << rhs.type_name;
  }
  else if (logFormatting) {
    ostr << "        <parameter value=\"" << rhs.type_name <<"\" />";
  }
  else {
    ostr << "            <parameter>" << endl
	 << "              <parameter value=\"" << rhs.type_name <<"\" name=\"TypeName\" />" << endl
	 << "              <parameter value=\"0.0\" name=\"Timeout\" />" << endl
	 << "            </parameter>";
  }
  return ostr;
}
...
parseArgs(int& argc, char* argv[])
{
  ...
  ACE_Get_Opt get_opts (argc, argv, "c:lt:x?");
  ...
  if (rc) {
    cerr << "usage: " << argv[0] << "[-t timeout [-c channel_name] [-v?]" << endl
	 ...
	 << "  -x xnl formatting as used by EventTimeoutFilterParamters" << endl
	 << "  -v verbose mode" << endl
...
int main(int argc, char * argv[])
{
  ...
  CosNotification::EventTypeSeq_var events = 
    proxy_supplier->
    obtain_offered_types(CosNotifyChannelAdmin::ALL_NOW_UPDATES_ON);
  ...
  public:
    ...
    {
      stringstream type;
      type << event.header.fixed_header.event_type;
 
      unsigned int size = offers.size();
      offers.insert(type.str());
