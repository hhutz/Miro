...
int main(int argc, char * argv[])
{
  ...
  char const * domainName = "Robot_Name";
  char const * typeName = "Event_Name";
  ...
  added[0].type_name = CORBA::string_dup(typeName);  
  ...
  {
    typedef std::map<string, int> EventMap;
    EventMap eventMap;
  ...
  public:
      ...
      throw()
    {
      ...
      if (eventMap.insert(make_pair(string((char const*)event.header.fixed_header.event_type.type_name),
				    cdr.total_length())).second == true) {

	EventMap::const_iterator first, last = eventMap.end();
	for (first = eventMap.begin(); first != last; ++first) {
	  cout << "type name: " << first->first << "    size: " << first->second << endl;
	}
	cout << endl;
      }

//       cout << "type name: "
// 	   << event.header.fixed_header.event_type.type_name << flush;

//       cout << "   size: " << cdr.total_length() << endl;
    }
