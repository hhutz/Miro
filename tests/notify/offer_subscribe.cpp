...
char const * NOTIFY_EVENT_CHANNEL = "NotifyEventChannel";
...
Pair a[] =   { {"foo", "bar0"}, {NULL, NULL} };
Pair b[] =   { {"foo", "bar0"}, {"foo", "bar1"}, {NULL, NULL} };
Pair c[] =   { {"foo", "barA"}, {"foo", "barB"}, {NULL, NULL} };

Pair * types[] = { a, b, c};

int const NUM_TYPES = sizeof(types) / sizeof(Pair *);

class OfferListener : public POA_CosNotifyComm::StructuredPushConsumer
{
  void 
...
class SubscriptionListener : public POA_CosNotifyComm::StructuredPushSupplier
{
  void
  ...
  void
  disconnect_structured_push_supplier() throw() 
  {}
};

class DummyConsumer : public POA_CosNotifyComm::StructuredPushConsumer
{
  void 
  push_structured_event(CosNotification::StructuredEvent const& /*event*/)
    throw()
...
class DummySupplier : public POA_CosNotifyComm::StructuredPushSupplier
{
  void
  subscription_change(CosNotification::EventTypeSeq const&,
                      CosNotification::EventTypeSeq const&)
    throw()
  {}
  void
